...
def filter_by_last_edited_time(items, date_str):
    ...
    return [i for i in items if target <= datetime.fromisoformat(i['last_edited_time'].replace('Z', '+00:00')) <= next_day]

def filter_inbox_tasks(tasks):
    inbox = []
    for t in tasks:
        status = t.get("properties", {}).get("Status", {}).get("status", {}).get("name", "")
        has_project = t.get("properties", {}).get("Projects", {}).get("relation", [])
        has_area = t.get("properties", {}).get("Areas/Resources", {}).get("relation", [])
...
def extract_linked_projects_and_areas(items):
    ...
    for item in items:
        ...
        for p in props.get("Projects", {}).get("relation", []):
            projects.add(p.get("id"))
        for a in props.get("Areas/Resources", {}).get("relation", []):
            areas.add(a.get("id"))
    return list(projects), list(areas)

def extract_tags_and_categories(items):
    tags = set()
    for item in items:
        props = item.get("properties", {})
        for multi in ["Tags", "Kategorie"]:
            for tag in props.get(multi, {}).get("multi_select", []):
...
def generate_gpt_summary(
    ...
    prompt = f"""Zusammenfassung für den {date_str}:
...
- Keine Auflistung einzelner Task- oder Notiztitel! Nur zählen für I. bis V.
